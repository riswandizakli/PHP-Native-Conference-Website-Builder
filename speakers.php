...
function uploadProfile($leadid)
{
    ...
    $namaStmtFileBaru = $leadid.'_speakers';//uniqid();
    $namaStmtFileBaru .= '.';
...
$ekstensi_diperbolehkan	= array('png','jpg','jpeg');
    ...
    move_uploaded_file($tmpStmtName, 'speakers/' . $namaStmtFileBaru);
			}
...
$idmap = $_GET['type'];
if($idmap != 'keynote' and $idmap != 'invited' and $idmap != 'welcome'){
    
    echo "<script>window.location = 'speakers.php?type=keynote'</script>";
 ...
 if(isset($_POST['tambah'])){
   require_once("config.php");
             ...
             $for6 = filter_input(INPUT_POST, 'for6', FILTER_SANITIZE_STRING);
                ...
                $profpic = uploadProfile($for1.$for2.$for3);
                      $for5 = $profpic;   
         ...
         $sql = "INSERT INTO speakers (conf, type, nama, dari, foto, pos)
            VALUES (:satu, :dua, :tiga, :empat, :lima, :enam)";
    $stmt = $db->prepare($sql);
    ...
    echo "<SCRIPT> //not showing me this
        alert('$message')
        window.location.replace('speakers.php');
    </SCRIPT>";
    }
					...
					<div class="row">
						<div class="col-md-12">
								...
								<div class="card-header">
								    	<div class="row mt--2">
						<div class="col-md-9">
						    	<h4 class="card-title">List Speakers (<?php echo $idmap;?>)</h4><br/>
						    </div>
								...
								<div class="card-body">
									<div class="table-responsive">
										<table id="multi-filter-select" class="display table table-striped table-hover" >
                                            ...
                                            $records = mysqli_query($db, "SELECT * FROM speakers where conf ='$conf' AND type='$idmap' order by pos");
                                            
                                              $records2 = mysqli_query($db, "SELECT * FROM speakers where conf ='$conf' AND type='$idmap' order by pos LIMIT 1000 OFFSET 1 ");
      $count = 1;
      
       
                                              $records3 = mysqli_query($db, "SELECT * FROM speakers where conf ='$conf' AND type='$idmap' order by pos desc");
            ...
            ?>
												<tr>
													...
													<td><img src="speakers/<?php echo $data['foto']; ?>" alt="" height="200">
</td>
													...
													<td><?php echo $data['nama']; ?></td>
													   ...
													   <form action="uprankbyid.php" method="POST" class="form-horizontal" enctype="multipart/form-data" onsubmit="return confirm('Make this item lower?');" style-form>
													         <input type="hidden" name="cate" value="speakers">
													...
													<td>
													   <form action="deletebyid.php" method="POST" class="form-horizontal" enctype="multipart/form-data" onsubmit="return confirm('Apakah anda yakin ingin menghapus ini?');" style-form>
													        <input type="hidden" name="aidi" value="<?php echo $data['id']; ?>">
													        <input type="hidden" name="idmap" value="<?php echo  $idmap; ?>">
							...
							<div class="card">
								<div class="card-header">
									<div class="card-title">Tambah Speakers Conference (<?php echo $idmap;?>) </div>
								</div>
							<div class="card-body">
								<div class="row">
								...
								<div class="form-group">
												<label for="email2">Nama Speakers
</label>
												<input name="for3" type="text" class="form-control" id="email2" placeholder="Name here.">
											<!--	<small id="emailHelp2" class="form-text text-muted">We'll never share your email with anyone else.</small>-->
											</div>
											
														 
								<div class="form-group">
												<label for="email2">Foto Speakers
</label>
												<input name="for5" type="file" class="form-control" id="email2" placeholder="image.">
											<!--	<small id="emailHelp2" class="form-text text-muted">We'll never share your email with anyone else.</small>-->
											</div>
											...
											<div class="col-md-6 col-lg-6">
											    	<div class="form-group">
												<label for="email2">Role Speakers / Association
</label>
												<input name="for4" type="text" class="form-control" id="email2" placeholder="Associate Profesor..">
											<!--	<small id="emailHelp2" class="form-text text-muted">We'll never share your email with anyone else.</small>-->
												<input name="for6" type="hidden" class="form-control" value="<?php echo $cate;?>">
											</div>
											    </div>
